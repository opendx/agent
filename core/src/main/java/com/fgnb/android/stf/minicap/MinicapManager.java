...
public class MinicapManager {
    ...
    /** 在网页上手机显示的高度 */
    private int screenDisplayHeight =Integer.parseInt(AppicationContextRegister.getApplicationContext().getEnvironment().getProperty("screenDisplayHeight"));
    ...
    public void startMiniCap() throws Exception {
        ...
        //实际屏幕分辨率
        String actualDeviceResolution = androidDevice.getDevice().getResolution();
    ...
    private String getScreenDisplayResolution() throws Exception {
        //比例
        double scale = (double)screenDisplayHeight / androidDevice.getDeviceHeight();
        int screenDisplayWidth = (int) Math.round( androidDevice.getDeviceWidth()*scale );
